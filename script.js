        ...
        .then(data => {
            if (data.news && data.last_updated) {
                ...
                if (lastUpdateEl) {
                    ...
                    lastUpdateEl.innerHTML = `<div style="text-align:center; line-height:1.6; width:100%; display:flex; flex-direction:column; align-items:center;">
                        <span>最新100条日媒发布的中国报道</span>
                        <span>上一次数据抓取时间：东京时间${formattedTime}</span>
                    </div>`;
                }
            } else {
